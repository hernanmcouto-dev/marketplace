...
export default function SanjulianPanel() {
  ...
  const loadProducts = async () => {
    try {
      const response = await fetch("/products-sanjulian.json");
  ...
  const handleScrape = async () => {
    ...
    addLog("Iniciando scraping de San Julián...");

    try {
      const response = await fetch("/api/scrape-sanjulian", { method: "POST" });
  ...
  return (
    <div style={{ minHeight: "100vh", backgroundColor: "#0f172a", color: "white" }}>
      <header style={{ background: "linear-gradient(135deg, #10b98180 0%, #0f172a 100%)", borderBottom: "2px solid #10b981", padding: "2rem 1rem" }}>
        <div style={{ maxWidth: "1200px", margin: "0 auto" }}>
          ...
          <h1 style={{ margin: 0, fontSize: "2rem", fontWeight: "bold" }}>🟢 San Julián</h1>
          ...
          {[
            ...
            <button
              ...
              style={{
                ...
                backgroundColor: activeTab === tab.key ? "#10b981" : "#1e293b",
                border: `2px solid ${activeTab === tab.key ? "#10b981" : "#334155"}`,
                borderRadius: "0.5rem",
                color: "white",
                cursor: "pointer",
